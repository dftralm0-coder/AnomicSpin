...
// Simple in-memory database (use JSON or DB for production)
const users = {}; // { userId: { wallet: 0, vault: 0, ghost: false } }
const adminIDs = ["YOUR_ADMIN_DISCORD_ID_HERE"]; // replace with your Discord ID
...
client.on('messageCreate', async (message) => {
  ...
  // ----- .bet command -----
    ...
    // Admin override ephemeral message
    for (let adminId of adminIDs) {
      const admin = await client.users.fetch(adminId);
      if (admin) {
        admin.send(`🎲 Player ${message.author.tag} bet ${amount} on ${color}. Roll: ${resultColor}.`);
      }
    }
  }
});
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  if (interaction.customId === 'ghost') {
    ...
    await interaction.reply({ content: 'Deposit is currently under development.', ephemeral: true });
  }

  else if (interaction.customId === 'withdraw') {
    await interaction.reply({ content: 'Withdraw is currently under development.', ephemeral: true });
  }

  else if (interaction.customId === 'give') {
    await interaction.reply({ content: 'Give chips is currently under development.', ephemeral: true });
  }
});
